...
sweep_configuration = {
    ...
    "parameters": {
        "total_timesteps": {"values": [300000, 400000, 500000]},
        "learning_rate": {"max": 1e-3, "min": 1e-4},
        "tau": {"max": 1.0, "min": 0.8}, 
        "buffer_size": {"values": [5000, 10000, 20000]},
        "end_e": {"values": [0.025, 0.05, 0.1, 0.2]},
        "exploration_fraction": {"values": [0.4, 0.5, 0.6]},
    },
}

sweep_id = wandb.sweep(sweep=sweep_configuration, project="cleanRL")
...
"""
    ...
    if args.track:
        ...
        wandb.init(
            ...
            config=sweep_configuration,    # vars(args)
            name=run_name,
            monitor_gym=True,
            save_code=True,
        )
    ...
    optimizer = optim.Adam(q_network.parameters(), lr=wandb.config.learning_rate)   #_ args.learning_rate
    target_network = QNetwork(envs).to(device)
    target_network.load_state_dict(q_network.state_dict())

    rb = ReplayBuffer(
        wandb.config.buffer_size,   #_ args.buffer_size
    ...
    for global_step in range(total_timesteps): #_ args.total_timesteps
        # ALGO LOGIC: put action logic here
        epsilon = linear_schedule(args.start_e, wandb.config.end_e, wandb.config.exploration_fraction * wandb.config.total_timesteps, global_step)
        #_ args.end_e, args.exploration_fraction * args.total_timesteps
        if random.random() < epsilon:
            actions = np.array([envs.single_action_space.sample() for _ in range(envs.num_envs)])
        else:
